#!/usr/bin/env python3
"""Compute the kernel-uprobe offset for capturing the camera AV-channel
password from a VTech APK at runtime.

Usage: python3 extract_password_offsets.py <split-apk-path>
...
Hooks `avClientStartEx(in_struct, out_struct)` in libAVAPIs.so. The first
arg is a pointer to a struct that has the password as a `char *` at
offset 0x18. A chained-deref uprobe (`+0(+0x18(%x0)):string`) reads the
password string at the moment the official VTech app opens the camera
live view.

Same machinery as `extract_license_offsets.py` — different lib + symbol.
"""

import struct
import sys

LIB_PATH_IN_APK = "lib/arm64-v8a/libAVAPIs.so"
...
def main():
    if len(sys.argv) != 2:
        print(
            "usage: extract_password_offsets.py <split-apk-path>\n"
            "  e.g.  extract_password_offsets.py archive/xapk_extracted/config.arm64_v8a.apk",
            file=sys.stderr,
        )
        sys.exit(1)
    apk_path = sys.argv[1]
    ...
    print("# Now extract the password (open the VTech app + tap your camera tile to trigger):", file=sys.stderr)
    print(f"#   adb push extract_password.sh /data/local/tmp/", file=sys.stderr)
    print(
        "#   adb shell su -c '/data/local/tmp/extract_password.sh "
